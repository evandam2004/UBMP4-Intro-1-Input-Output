 ...
 * The board waits until one of which of the buttons that are pressed first
 * and waits until it is done, then if you want to the next pattern you will
 * need to click the other switch then.
 * 
 ...
 *  The waveform would look like is that depending on how loud the sound is,
 * say the delay on one switch is 500 and the other is 800, this would be two 
 * differnt levels of sound, meaning that  there would be two lines to be seen
 * since you are pressing two differnt buttons with two differnt sounds 
 * 
 ...
 * towards the light as it has to pass through both the buttons, and the one 
 * button stops current.
 * 
 * 
